...
/**
 * Per-platform spacing value for the Home wallet cluster.
 *
 * The per-combination offsets below were measured and tuned on an iPhone, and
 * they are not platform-neutral: Android composes the same carousel at a
 * different height, so the iPhone numbers visibly mis-space it. Rather than
 * re-tune nine constants twice, each value carries the Android number it had
 * before the iOS pass, so Android stays byte-for-byte as it shipped in 0.1.10
 * and only iOS moves.
 *
 * If you re-tune iOS, do NOT change the second argument without measuring on
 * an Android device. It is a record of what shipped, not a guess.
 */
const homeLift = (ios: number, android: number): number =>

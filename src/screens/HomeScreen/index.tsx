          ...
          (
            <>
              ...
              */}
              ...
              <Animated.View style={{ opacity: enterAnim, transform: [{ translateY: -25 /* lift Total Balance card 25pt up */ }, { translateY: (!isAuth && !isLoading && !isStrikeAuth && !isArkAuth) ? 2 /* no LN wallet: +30 vs prior -28, drop the Total Balance box + Unlock-Lightning card + send/receive so they stop overlapping the "Total Assets" header */ : (!isLoading && isStrikeAuth && isArkAuth && !isAuth) ? -33 : (!isLoading && isAuth && isStrikeAuth && isArkAuth) ? -18 : (!isLoading && isAuth && isStrikeAuth && !isArkAuth) ? 7 : (!isLoading && isStrikeAuth && !isAuth && !isArkAuth) ? -23 : (!isLoading && isAuth && !isStrikeAuth && !isArkAuth) ? -18 : (!isLoading && !isAuth && !isStrikeAuth && isArkAuth) ? -38 : (!isLoading && isAuth && !isStrikeAuth && isArkAuth) ? -30 /* Coinos+Ark: balance box down a touch toward the card, but keep clear space above the cards (tighten the 2-box cluster modestly) */ : -53 }, { translateY: enterTranslate }] }}>
                <BalanceView

...
# .example templates so the build always completes. The codegen prestep avoids
# RN's cold-build CMake race.
...
add_note "The app compiles config values into the binary (push-relay key in the"
add_note "JS bundle, .env web client ID in BuildConfig). If those inputs differ from"
add_note "the production build's values, the bundle differs by exactly those constants"
add_note "even when the toolchain output is otherwise reproducible. See"
add_note "walletscrutiny/README.md."
